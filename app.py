...
ROLE_REQUIREMENTS = {
    ...
    "Data Scientist": [
    "Python", "R", "SQL", "Rachine Learning", "Statistics", "Data Visualization",
...
def setup_agent(config):
    ...
    if not config["openai_api_key"]:
        st.error("A Please enter your OpenAI API Key in the sidebar.")
        return None
    
    # Initialize or update the agent with the API key
    if st.session_state.resume_agent is None:
        st.session_state.resume_agent = ResumeAnalysisAgent(api_key=config)
        ["openai_api_key"]
    else:
...
def analyze_resume(agent, resume_file, role, custom_jd=None):
    ...
    try:
        with st.spinner("🔍 Analyzing resume... This may take a minute."):
            if custom_jd:
                result = agent.analyze_resume(resume_file, custom_id=custom_jd)
            else:
...
def generate_interview_questions(agent, question_types, difficulty, num_questions):
    ...
    try:
        with st.spinner("Generating personalized interview questions..."):
            questions = agent.generate_interview_questions(question_types,
...
def get_improved_resume (agent, target_role, highlight_skills):
    ...
    try:
        with st.spinder("Creating improved resume..."):
        ...
        with col2:
            if st.button("🔍Analyze Resume", type="primary"):
                if agent and uploaded_resume:
                    # Just store the result, don't display it      
    ...
    with tabs[2]:
        if st.session_state.resume_analyzed and st.session_state.resume_agent:
            ui.interview_questions_section(
                has_resume=True,  # Explicitly set to True since we checked
